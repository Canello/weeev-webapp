...
const IdeaNotFound = () => {
    return (
        <>
            ...
            <h1 className="idea-not-found font-headline-mobile-1 color-n-20">
...
function App() {
    ...
    const [isRedirectingToHttps, setIsRedirectingToHttps] = useState(
        isDevMode ? false : true
    );
    ...
    useEffect(() => {
        ...
        if (window.location.protocol !== "https:") {
            window.location.replace(
                "https://weeev.fun" + window.location.pathname
            );
        } else {
    ...
    if (isRedirectingToHttps || isLoadingUserToken || isFetchingUser) {
        return (
            <div className="InitialLoadingScreen bg-b-white">
                ...
                <img className="weeev-icon" src={weeevIcon} alt="weeev logo." />
            </div>
        );
    }
    return (
